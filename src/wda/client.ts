import { WdaError, WdaUnavailableError } from "#/errors";
import type { Logger } from "#/exec";

/**
 * The four places WebDriverAgent's failures need words that this module cannot
 * know.
 *
 * Every one of them ends in advice about a *runner* — how to start it, how to
 * authorize it, which diagnostics tool reports it — and none of that is the same
 * on a phone and in a simulator: one needs a signed build and a Settings toggle
 * nobody can flip from this Mac, the other needs an `xcodebuild` command and
 * nothing else. The protocol is identical; only the fix differs. So the fixes
 * arrive from the server.
 *
 * All optional. Omitting them costs a remedy, never a wrong one.
 */
export type WdaRemedies = {
  /** WebDriverAgent did not answer at all. */
  unavailable?: string;
  /** It answered, but the XCTest lane is refusing to act. */
  notAuthorized?: string;
  /** It has a session but can see no foreground app — usually the same cause. */
  noForegroundApp?: string;
  /** An element was not found; names the tool that shows what is on screen. */
...
export type WdaOptions = {
  ...
  /** Server-supplied copy for the failures whose fix is target-specific. */
  remedies?: WdaRemedies | undefined;
};
...
export class WdaClient {
  ...
  private readonly remedies: WdaRemedies;
  /**
      ...
      const cause =
        err instanceof Error
          ? err.name === "AbortError"
            ...
            : err.message
          : String(err);
      throw new WdaUnavailableError(base, cause, this.remedies.unavailable);
    } finally {
      clearTimeout(timer);
    }
    ...
    if (!res.ok || failure) {
      throw new WdaError(
        `WebDriverAgent ${method} ${path} failed: ${failure?.message ?? failure?.error ?? res.statusText}`,
        {
          ...
          ...(this.remedyFor(failure) ? { remedy: this.remedyFor(failure)! } : {}),
        },
      );
    }
   ...
   */
  private remedyFor(failure: WdaValueError | undefined): string | undefined {
    if (failure && isNotAuthorized(failure.message)) return this.remedies.notAuthorized;
    switch (failure?.error) {
      case "no such element":
        return this.remedies.noSuchElement;
        ...
        if (retry instanceof WdaError && isStaleSession(retry) && retry.remedy === undefined) {
          throw new WdaError(
              ...
              "this is not a stale handle: WebDriverAgent cannot see any foreground application.",
            {
              ...
              ...(this.remedies.noForegroundApp ? { remedy: this.remedies.noForegroundApp } : {}),
            },
          );
        }
        throw retry;
 ...
 * own. Exported so a server's diagnostics tool classifies its probe with the
 * same rule the tools do, rather than a second one that can disagree with it.
 */
